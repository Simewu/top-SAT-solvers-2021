...
void 
kissat_init_additional(kissat * solver)    
{
  solver->is_glue_var = calloc(solver->vars,sizeof(bool)); 
  solver->is_decision = calloc(solver->vars,sizeof(bool)); 
  solver->in_glue =  calloc(solver->vars,sizeof(unsigned)); 
  solver->total_in_glue = 0;
  solver->max_bf = -1;
  solver->gbumped = 0;
  solver->stable_g = 0;
}
