...
void
bump_glue_vars(kissat *solver, unsigned v)
{
  solver->is_glue_var[v]=false;
  heap *scores = &solver->scores;
  solver->gbumped++;
  const double old_score = kissat_get_heap_score (scores, v);
  double bf = (double) solver->in_glue[v] / (double) solver->stable_g;
  if(bf>solver->max_bf)
    solver->max_bf = bf;
  const double new_score = old_score + old_score*bf;
  //printf("====>%d %f %f %f %f\n",v, bf, solver->max_bf, old_score, new_score);
  ...
  if (solver->stable)
      ...
      for (unsigned i = j; i != old_size; i++)
	{
	  ...
	  if (level <= new_level)
	    ...
	    {
        if(solver->is_glue_var[IDX(lit)])
        //   //&& solver->is_decision[IDX(lit)]) 
            bump_glue_vars(solver, IDX(lit));
	      unassign (solver, values, lit);
	      add_unassigned_variable_back_to_heap (solver, scores, lit);
        unassigned++;
        //solver->is_decision[IDX(lit)]=false;        
	    }
	}
    }
  else
