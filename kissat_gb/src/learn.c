...
void
track_glu_var (kissat *solver)
{
  for(unsigned i=0;i<SIZE_STACK(solver->clause.lits);i++)
  {
    unsigned var = IDX(PEEK_STACK (solver->clause.lits, i));
    solver->is_glue_var[var]=true;
    solver->in_glue[var]++;
    solver->total_in_glue++;    
  }
...
kissat_learn_clause (kissat * solver)
{
  ...
  if(glue==1) // solver->levels does not contain the current level. glue=1 clauses are glue clasues (Contains two literal blocks, one from current)
  {
    ADD(num_glue,1);
    if(solver->stable)
    {
      track_glu_var(solver);
      solver->stable_g++;
    }
  }
  ADD(total_glue,glue); 
